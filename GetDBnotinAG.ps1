...
#Local Instance Name here - Primary Replica
$serverinstance = "ORF-SQL14-21"
$Database = "Master"
$SQL = "SELECT Name From sys.databases WHERE database_id > 4 AND group_database_id IS NULL AND NAME not in ('Admin', 'DMV_DATA', 'NUM259')"
$RES = Invoke-Sqlcmd -ServerInstance $serverinstance -Database $Database -Query $SQL -ErrorAction Stop -ErrorVariable sqlConnectError
if($sqlConnectError.count -ne 0){
    $databasestoAG = $RES.NAME -join ","
    
    $primaryReplica = $serverinstance

    #Provide Secondary Replica Instances in Comma Seperated Format
    $secondaryReplica = "ORF-SQL14-20,ORF-SQL14-22"
    SetupAG -PrimaryReplicaName $primaryReplica -SecondaryReplicaNames $secondaryReplica -DB_NAMES $databasestoAG
} Else {
    Write-Host "Error Choosing databases to be added"
    EXIT
}
